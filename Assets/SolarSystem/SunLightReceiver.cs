...
namespace GalaxyExplorer
{
    public class SunLightReceiver : MonoBehaviour
    {
        ...
        private void Awake()
        {
            ...
            if (SetMaterials && currentRenderer.sharedMaterial.name.Equals("Moon"))
            {
        ...
        {
            if (SetMaterials && FindSunIfNeeded())
            {
        ...
        {
            if (SetMaterials && currentRenderer.sharedMaterial.HasProperty("_SunDirection") || moonMaterial)
            {
                if (moonMaterial)
                {
